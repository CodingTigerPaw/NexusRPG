...
import type { CharacterActionDefinition, CharacterSystemDefinition } from './types';
...
const skillCheck: CharacterActionDefinition = {
  ...
  createDiceNotation(input, character) {
    return input.skill && hasNumericCharacterValue(character, input.skill) ? '1d100' : null;
  },
  resolveMechanics(input, roll, state) {
    return {
      ...
      target: typeof state.data.target === 'number' ? state.data.target : undefined,
      roll: roll.total,
      successLevel:
        typeof state.data.successLevel === 'string' ? state.data.successLevel : undefined,
      status: state.status,
...
export const coc5SystemDefinition: CharacterSystemDefinition = {
  ...
  shortDescription: 'Klasyczny investigator z profesją i cechami opartymi o rzuty.',
